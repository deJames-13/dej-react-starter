...
function FormInput({ label, refer, meta, ...inputProps }) {
  return (
    <div>
      <label class={`${meta?.touched && meta?.error ? 'text-error' : 'text-base-content'}`}>{label}</label>
      <input className={`input input-bordered ${meta?.touched && meta?.error ? 'input-error' : 'input-primary'} w-full`} {...inputProps} />
