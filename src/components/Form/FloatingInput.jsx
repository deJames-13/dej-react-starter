...
function FloatingInput({ label, refer, meta, ...inputProps }) {
  return (
    <div>
      <div className={`relative border ${meta?.touched && meta?.error ? 'border-error' : 'border-gray-300'} overflow-clip`}>
        <input
          type='text'
          ref={refer}
          {...inputProps}
          ...
          } appearance-none  focus:border-b-2 focus:outline-none focus:ring-0 focus: ${meta?.touched && meta?.error ? 'border-error' : 'border-primary'}/80 peer`}
          placeholder=' '
          autoComplete='off'
        />
        <label
          htmlFor={inputProps.id ?? 'floating__filled'}
          className={`absolute text-sm ${meta?.touched && meta?.error ? 'text-error' : ''} duration-300 transform -translate-y-4 scale-75 top-4 z-10 origin-[0] start-2.5 peer-focus:${
